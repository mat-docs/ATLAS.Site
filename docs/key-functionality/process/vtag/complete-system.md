...
When the car goes out, ATLAS's Data recorder will start recording automatically, and so should the VTS Recorder (see **Setting up the vTAGServer**). You will see the data appearing on your screen, together with the virtual data from your model.
...
The use of the vTAGSrv Control is the same as during the live modes.
...
4. Open a new vTAGSrv control in that same layer (FILE -> NEW and then choose vTAGSrv Control)
5. Link it to the right ECU; You have to choose your IP address [tab OPTIONS, choose, click button apply]
6. Define the time span; You can choose the Time period [tab DATA SOURCE, drop-down menu Session, Current Lap, Fastest Lap, All timed laps, Current time base] and choose start
7. Start the simulation. ATLAS will display data.
...
Each time the Historical playback is run, a new session file will be produced by the ATLAS API.

For both Complete and Standalone Systems, the same version numbering scheme is applied in which the `nnn` in the session name format, `xxx.vvv.nnn.ssv`, is incremented by 1, ensuring that the smallest number is used to keep the files unique within the specific path.

## Opening Multiple Sessions

In the ATLAS Open Session Dialog (OSD), you can open both the actual and simulated data together into a single layer. A column for each unique `vvv` identifier in the `ssv` file format structure is present.
