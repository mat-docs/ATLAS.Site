...
with the same effective name, a target missing `Host`/`Port`, or a WAL setting below its minimum.
See
...
**Cause**: the target's connection attempts are failing (unreachable `Host`/`Port`). Retries for
a target are unbounded — a target can be down indefinitely without affecting any other target —
and back off exponentially: each retry waits `min(InitialDelayMs * 2^attempt, MaxDelayMs)` plus a
small random jitter (up to `JitterMs`), so retries get progressively less frequent up to
`MaxDelayMs` (default `30000` ms).

**Fix**: check the service's logs for connection/reconnection errors for that target's name, and
confirm the target's `Host`/`Port` is actually reachable from where Broadcast Service is running.
Once the target does connect, `broadcast_target_up` flips to `1` and `broadcast_reconnect_total`
records how many reconnect attempts it took.
...
`Targets.BridgeConfigs` lets compaction proceed for the ones that remain.
