...
The _MAT.OCS.RTA.Model_ [NuGet Package](../../downloads.md#nuget-packages) has a `Chunk` class that holds the data in serialized and compressed form.
...
The _MAT.OCS.RTA.Services.AspNetCore_ [NuGet Package](../../downloads.md#nuget-packages) provides formatters to send a `ChunkedResult` (wrapping a `Chunk` stream) back to the client as the [`application/vnd.mat.protobuf+chunked` wire-format](../protobuf/net_chunks.md).

## Constraints

This section describes some important constraints, using RFC&nbsp;2119 language (MUST, SHOULD, etc).

These are important if you are implementing a [Data Service](../../../../key-functionality/integrate/rta/introduction/data-services.md).
...
:   Round the request time-bounds (if any) outwards &mdash; generally just by dropping precision.  
    The _MAT.OCS.RTA.Services_ [NuGet Package](../../downloads.md#nuget-packages) provides a `ChunkTime` utility to help with this.
